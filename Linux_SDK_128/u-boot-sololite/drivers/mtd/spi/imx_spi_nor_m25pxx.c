...
static int wait_till_write_ready(struct spi_flash *flash)
{
	int sr;
	int times = 10000;
	...
	do {
		...
		g_tx_buf[1] = 0x70;
		if(spi_xfer(flash->spi, 2 << 3, g_tx_buf, g_rx_buf, SPI_XFER_BEGIN | SPI_XFER_END) != 0) {
			break;	
		}
		...
		if (sr < 0)
			...
			/* An error occured, clear the flag status registers */	
			g_tx_buf[0] = 0x50;
			if(spi_xfer(flash->spi, 1 << 3, g_tx_buf, g_rx_buf, SPI_XFER_BEGIN | SPI_XFER_END) != 0) {
				break;	
			}
		}
...
static int erase_sector(struct spi_flash *flash, u32 offset)
{
	...
	spi_nor_cmd_1byte(flash, OPCODE_EN4B);
	udelay(1000);

	g_tx_buf[1]= 0x70;
	spi_xfer(flash->spi, 2 << 3, g_tx_buf, g_rx_buf, SPI_XFER_BEGIN | SPI_XFER_END);
	...
	if (spi_xfer(flash->spi, (5 << 3) , array, g_rx_buf,
				SPI_XFER_BEGIN | SPI_XFER_END)) {
		return -1;
	}

	if(wait_till_write_ready(flash))
		return -1;

	WRITE_ENABLE(flash);
        spi_nor_cmd_1byte(flash, OPCODE_EX4B);
...
static int _fsl_spi_write(struct spi_flash *flash, const void *buf, int len, int addr)
{
	...
	while (count) {
		d = txer;
		l = count > (SPI_FIFOSIZE - 5) ?
		    SPI_FIFOSIZE - 5 : count;
...
/* Read Flag status register */
        ...
        if(spi_xfer(flash->spi, 2 << 3, g_tx_buf, g_rx_buf, SPI_XFER_BEGIN | SPI_XFER_END) != 0) {
                ...
                printf("Error:spi write failed %s() %d\n", __func__, __LINE__);
