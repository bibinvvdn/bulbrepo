...
all_services="hostname devfsd filesystems syslog network depmod modules inetd portmap dropbear boa smb dhcpd settime qtopia watchdog gtk2 pango"
all_services_r="pango gtk2 watchdog qtopia settime dhcpd smb boa dropbear portmap inetd modules depmod network syslog filesystems devfsd hostname"

cfg_services="hostname $devfsd filesystems $syslog $network depmod modules $inetd $portmap $dropbear $boa $smb $dhcpd $settime $qtopia $watchdog $gtk2 $pango"

cfg_services_r="$pango $gtk2 $watchdog $qtopia $settime $dhcpd $smb $boa $dropbear $portmap $inetd modules depmod $network $syslog filesystems $devfsd hostname"
...
KERNEL_VER=`strings $TOP/rootfs/boot/vmlinux | perl -n -e 'print($1), exit(0) if m,Linux version ([\S]+),'`
if [ -n "$KERNEL_VER" ]
then
...
# copy the release info file (if it exists)
if [ -f "$TOP/RELEASE_INFO" ]
then
    mkdir -p $RPM_BUILD_ROOT/%{pfx}/etc
    cp $TOP/RELEASE_INFO $RPM_BUILD_ROOT/%{pfx}/etc/ltib-release
fi
