...
cmgr_return_t msg_frm_nw_wlan_get(
                cmgr_s8bit          *p_buf,
...
cmgr_return_t timeout_mesg_wlan_get(cmgr_ecode_t *p_ecode);

CMD_INTERFACE_T *payload_global;

api_return_e Web_Get_Wlan_Cfg_Api(WLAN_CFG_T *wlan_cfg, char *wlan_cfg_err)
{
        ...
        if (!p_cmd) {
                WS_TRACE("[WEB_WLAN_GET]Malloc fails");
                exit (1) ;
        }

        p_cmd->module_id = MOD_ID_WEB_SRV; // Setting module id in header from web_api_common.h
        p_cmd->cmd_opcode = WEB_CFGM_WLAN_CFG_GET; // Setting opcode from web_api_opcode.h
        ...
        if(CMGR_SUCCESS != cmgr_init_conn_lib(MOD_ID_WEB_SRV, msg_frm_nw_wlan_get,
                                timeout_mesg_wlan_get, (cmgr_s8bit*)"Web Server",
                                &cmgr_error))
        {
        ...
        if(payload_global->cmd_exec_status == API_CMD_PROCESSED){
                memcpy(wlan_cfg,payload_global->buff,sizeof(WLAN_CFG_T));
                return SUCCESS;
        }
        else{
                ...
                cmgr_ecode_t        *p_ecode)
{
        ...
        if (CFGM_WEB_WLAN_CFG_GET != payload_global->cmd_opcode)
        {
                WS_TRACE("[WEB_WLAN_GET] Err. Received Invalid message\n") ;
        }
...
main()
{
	...
	printf("Content-Type:text/html\n\n");
     /*   printf("Before Calling API\n");  */
        if(SUCCESS == Web_Get_Wlan_Cfg_Api(&info_nw, err)){
/*		printf("wire=%s %d %s %u %d %d %d %s %s %d",info_nw.ap_ssid,info_nw.ap_tx_pwr,info_nw.sta_ssid,info_nw.sta_tx_pwr,info_nw.sta_sec,info_nw.sta_wep_keyno,info_nw.sta_wep_secret,info_nw.sta_wpa_secret,info_nw.sta_wpa_enc);
*/
		printf("wire=%s %d %s",info_nw.sta_ssid,info_nw.sta_sec,info_nw.sta_wpa_secret);
        /*        printf("After Called API_mmmmm\n");  
                printf("Got structure from cfg:::\n");
                printf("ap_ssid         : %s\n", info_nw.ap_ssid);
                printf("ap_chn	    	: %u\n", info_nw.ap_chnl);
                printf("stat_chn    	: %u\n", info_nw.sta_chnl);
                printf("ap_tx_per       : %d\n", info_nw.ap_tx_pwr);
                printf("sta_wpa_secret  : %s\n", info_nw.sta_wpa_secret);
        }else{
