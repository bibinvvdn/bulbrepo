...
cmgr_return_t msg_frm_nw_bulbstats_set(
		cmgr_s8bit          *p_buf,
...
cmgr_return_t timeout_mesg_bulbstats_set(cmgr_ecode_t *p_ecode);

CMD_INTERFACE_T *payload_global;

api_return_e Web_Set_BULBSTATS_Cfg_Api(BULBSTATS_CFG_T *bulbstats_cfg, char *bulbstats_cfg_err)
{

	CMD_INTERFACE_T *p_cmd;
	const unsigned short buff_size = sizeof(CMD_INTERFACE_T) + sizeof(BULBSTATS_CFG_T); //Calculating the buffer size which includes header and payload
	...
	if (!p_cmd) {
		WS_TRACE("[WEB_BULBSTATS_SET] Malloc fails");
		exit (1) ;
	}

	p_cmd->module_id = MOD_ID_WEB_SRV; // Setting module id in header from web_api_common.h
	p_cmd->cmd_opcode = WEB_CFGM_BULBSTATS_CFG_SET; // Setting opcode from web_api_opcode.h
	...
	memcpy(p_cmd->buff,bulbstats_cfg,sizeof(BULBSTATS_CFG_T));

	if(CMGR_SUCCESS != cmgr_init_conn_lib(MOD_ID_WEB_SRV, msg_frm_nw_bulbstats_set,
				timeout_mesg_bulbstats_set, (cmgr_s8bit*)"Web Server",
				&cmgr_error))
	{
	...
	if(payload_global->cmd_exec_status == API_CMD_PROCESSED){
		...
		strcpy(bulbstats_cfg_err,payload_global->buff);
		return FAILURE;
	}
}
		...
		cmgr_ecode_t        *p_ecode)
{
	...
	if (CFGM_WEB_BULBSTATS_CFG_SET != payload_global->cmd_opcode)
	{
		WS_TRACE("[WEB_BULBSTATS_SET] Err. Received Invalid message\n") ;
	}
...
main()
{
	BULBSTATS_CFG_T info_sys;
	char err[100];
	int qlen=0;
	fprintf(stderr,"Bulb stat Before Calling API\n");

	char *method = getenv("REQUEST_METHOD");
	char *lenstr = getenv("CONTENT_LENGTH");
//	char *qstr=getenv("QUERY_STRING");
//	char *qstr=" ONBULB=100&OFFBULB=1&DIMBULB=0 ";	
	...
	for(i=1;i<qlen-1;i++)
	{
		...
		if(! strncmp(variable,"ONBULB",6)) {
			info_sys.stat_on = atoi(value);
		} else if (! strncmp(variable,"OFFBULB",7)) {
			info_sys.stat_off = atoi(value);
		} else if (! strncmp(variable,"DIMBULB",7)) {
			info_sys.stat_dim = atoi(value);
		}
	}

	fprintf(stderr,"%d,%d,%d\n",info_sys.stat_on,info_sys.stat_off,info_sys.stat_dim);


	// API call
	if(SUCCESS == Web_Set_BULBSTATS_Cfg_Api(&info_sys, err)){
