...
cmgr_return_t msg_frm_nw_scan_get(
                cmgr_s8bit          *p_buf,
...
cmgr_return_t timeout_mesg_scan_get(cmgr_ecode_t *p_ecode);

CMD_INTERFACE_T *payload_global;

api_return_e Web_Get_Scan_List_Api(SCAN_CFG_T *scan_cfg, char *scan_list_err)
{
        CMD_INTERFACE_T *p_cmd;
        const unsigned long buff_size = sizeof(CMD_INTERFACE_T) + sizeof(SCAN_CFG_T); //Calculating the buffer size which includes header and payload
        ...
        if (!p_cmd) {
                WS_TRACE("[WEB_SCAN_GET]Malloc fails");
                exit (1) ;
        }

        p_cmd->module_id = MOD_ID_WEB_SRV; // Setting module id in header from web_api_common.h
        p_cmd->cmd_opcode = WEB_CFGM_SCAN_LIST_GET; // Setting opcode from web_api_opcode.h
	...
	memcpy(p_cmd->buff,scan_cfg,sizeof(SCAN_CFG_T));

        if(CMGR_SUCCESS != cmgr_init_conn_lib(MOD_ID_WEB_SRV, msg_frm_nw_scan_get,
                                timeout_mesg_scan_get, (cmgr_s8bit*)"Web Server",
                                &cmgr_error))
        {
        ...
        if(payload_global->cmd_exec_status == API_CMD_PROCESSED){
                memcpy(scan_cfg,payload_global->buff,sizeof(SCAN_CFG_T));
                return SUCCESS;
        }
        else{
                strcpy(scan_list_err,payload_global->buff);
                printf("Err Msg: %s\n",scan_list_err);
                return FAILURE;
        }
}
                ...
                cmgr_ecode_t        *p_ecode)
{
        ...
        if (CFGM_WEB_SCAN_LIST_GET != payload_global->cmd_opcode)
        {
                WS_TRACE("[WEB_SCAN_GET] Err. Received Invalid message\n") ;
        }
...
main()
{
    ...
    /*    printf("Before Calling API\n");  */
        if(SUCCESS == Web_Get_Scan_List_Api(&info_nw, err)){
	   ...
	   */
		printf("{");
		printf("\"scan\":[");
		for (i = 0; i < info_nw.no_count-1 ; i++) {
			printf("\"%s\",",info_nw.scan[i].ssid);
	 	}  
		printf("\"%s\"",info_nw.scan[info_nw.no_count-1].ssid); 
		printf("]");
//		printf("\"EOF\":\"EOF\"]");
		printf("}");
        }else{
