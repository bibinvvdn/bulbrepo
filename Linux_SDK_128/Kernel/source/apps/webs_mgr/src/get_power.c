...
cmgr_return_t msg_frm_nw_power_get(
                cmgr_s8bit          *p_buf,
...
cmgr_return_t timeout_mesg_power_get(cmgr_ecode_t *p_ecode);

CMD_INTERFACE_T *payload_global;

api_return_e Web_Get_Power_Cfg_Api(POWER_CFG_T *power_cfg, char *power_cfg_err)
{
        ...
        if (!p_cmd) {
                WS_TRACE("[WEB_POWER_GET]Malloc fails");
                exit (1) ;
        }

        p_cmd->module_id = MOD_ID_WEB_SRV; // Setting module id in header from web_api_common.h
        p_cmd->cmd_opcode = WEB_CFGM_POWER_CFG_GET; // Setting opcode from web_api_opcode.h
        ...
        if(CMGR_SUCCESS != cmgr_init_conn_lib(MOD_ID_WEB_SRV, msg_frm_nw_power_get,
                                timeout_mesg_power_get, (cmgr_s8bit*)"Web Server",
                                &cmgr_error))
        {
        ...
        if(payload_global->cmd_exec_status == API_CMD_PROCESSED){
                memcpy(power_cfg,payload_global->buff,sizeof(POWER_CFG_T));
                return SUCCESS;
        }
        else{
                strcpy(power_cfg_err,payload_global->buff);
                printf("Err Msg: %s\n",power_cfg_err);
                return FAILURE;
        }
}
                ...
                cmgr_ecode_t        *p_ecode)
{
        ...
        if (CFGM_WEB_POWER_CFG_GET != payload_global->cmd_opcode)
        {
                WS_TRACE("[WEB_SRV_GET] Err. Received Invalid message\n") ;
        }
...
main()
{
	...
	printf("Content-Type:text/html\n\n");
      /*  printf("Before Calling API\n"); */
        if(SUCCESS == Web_Get_Power_Cfg_Api(&info_p, err)) {
		printf("[{");
		printf("\"power\":\"%f\"",info_p.pwr);
		printf("}]");
        }else{
